...
# Đọc ma trận từ file
def load_matrix_from_file(filename="matrix_maze.txt"):
...
# Thuật toán A* với hiển thị từng bước
def a_star(grid, start, goal, screen):
    size = len(grid)
    open_set = []
    g_score = {start: 0}
...
def draw_grid(screen, grid, open_set=None, closed_set=None, start=None, goal=None, path=None, total_weight=None):
    ...
    # Hiển thị giá trị ô, bỏ qua ô có giá trị 999
    for i in range(n):
        for j in range(n):
            if grid[i][j] != 999:  # Không hiển thị chữ nếu ô là 999
                rect = pygame.Rect(j * cell_size, i * cell_size, cell_size, cell_size)
                weight = grid[i][j]
...
def main():
    ...
    grid = load_matrix_from_file("matrix_maze.txt")
    start, goal = (0, 0), (n - 1, n - 1)
    path, total_weight = a_star(grid, start, goal, screen)
    ...
    output_dir = os.path.join("RESULT", "Weight_Maze")
